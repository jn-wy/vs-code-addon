...
- blockquote text: per-theme, tied to each theme's own accent color, darkened (accent 85% / black 15%, solid — same recipe as the marker below) over the paper background (Deep Raspberry `#831f51`, Deep Teal `#07625b`, Soft Cobalt `#22488c`) — replaces a shared global magenta (`#c121a4`) that was hue-isolated from the rest of the palette, then briefly an 85%-alpha accent tint that read too close in value to inline code.
...
code --install-extension jn-themes-0.3.9.vsix
```
